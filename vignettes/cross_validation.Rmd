...
vignette: >
  %\VignetteIndexEntry{cross_validation}
...
The dataset `elec_price` is a hourly `tsibble` with day-ahead electricity spot prices in [EUR/MWh] from the ENTSO-E Transparency Platform. The dataset contains time series data from 2019-01-01 to 2019-12-31 for four european bidding zones: 
...
The function `summarise_split()` provides a summary table of the partitioning into training and testing with the corresponding start and end (as date and index) for each split. This is very useful to identfy specific splits by date. For example, if a holiday falls into a specific testing slice or not. 
